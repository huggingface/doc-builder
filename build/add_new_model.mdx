...
- local: stepbystep-recipe-to-add-a-model-to-transformers
  sections:
  - local: optional-theoretical-aspects-of-brandnewbert
    title: 1. (Optional) Theoretical aspects of BrandNewBert
  - local: next-prepare-your-environment
    title: 2. Next prepare your environment
  - local: run-a-pretrained-checkpoint-using-the-original-repository
    title: 3.-4. Run a pretrained checkpoint using the original repository
  - local: port-brandnewbert-to-transformers
...
# [How to add a model to 🤗 Transformers?](#how-to-add-a-model-to-transformers)
...
## [General overview of 🤗 Transformers](#general-overview-of-transformers)
...
A good first starting point to better understand the library is to read the [documentation of our philosophy](/docs/transformers/master/en/philosophy.html). As a result of our way of working, there are some choices that we try to apply to all models:
...
[PreTrainedModel](/docs/transformers/master/en/main_classes/model.html#transformers.PreTrainedModel), and [PretrainedConfig](/docs/transformers/master/en/main_classes/configuration.html#transformers.PretrainedConfig). For exemplary purposes, we will
call the model to be added to 🤗 Transformers `BrandNewBert`.

Let's take a look:

<img src="./imgs/transformers_overview.png"/>
...
[PreTrainedModel](/docs/transformers/master/en/main_classes/model.html#transformers.PreTrainedModel). The important functionalities that are automatically provided to every new
model are [from_pretrained()](/docs/transformers/master/en/main_classes/model.html#transformers.PreTrainedModel.from_pretrained) and
[save_pretrained()](/docs/transformers/master/en/main_classes/model.html#transformers.PreTrainedModel.save_pretrained), which are used for serialization and deserialization. All of the
...
[PreTrainedModel](/docs/transformers/master/en/main_classes/model.html#transformers.PreTrainedModel), and thus can be accessed via the `config` attribute for all classes
...
[PretrainedConfig](/docs/transformers/master/en/main_classes/configuration.html#transformers.PretrainedConfig). Note that the configuration and the model are always serialized into two
different formats - the model to a _pytorch_model.bin_ file and the configuration to a _config.json_ file. Calling
[save_pretrained()](/docs/transformers/master/en/main_classes/model.html#transformers.PreTrainedModel.save_pretrained) will automatically call
[save_pretrained()](/docs/transformers/master/en/main_classes/configuration.html#transformers.PretrainedConfig.save_pretrained), so that both model and configuration are saved.
...
## [Step-by-step recipe to add a model to 🤗 Transformers](#stepbystep-recipe-to-add-a-model-to-transformers)
...
### [1. (Optional) Theoretical aspects of BrandNewBert](#optional-theoretical-aspects-of-brandnewbert)
...
-  What type of model is *brand_new_bert*? BERT-like encoder-only model? GPT2-like decoder-only model? BART-like
  encoder-decoder model? Look at the [model_summary](/docs/transformers/master/en/model_summary.html) if you're not familiar with the differences between those.
...
### [2. Next prepare your environment](#next-prepare-your-environment)
...
### [3.-4. Run a pretrained checkpoint using the original repository](#run-a-pretrained-checkpoint-using-the-original-repository)
...
### [5.-14. Port BrandNewBert to 🤗 Transformers](#port-brandnewbert-to-transformers)
