...
A good first starting point to better understand the library is to read the [documentation of our philosophy](philosophy.html). As a result of our way of working, there are some choices that we try to apply to all models:
...
[PreTrainedModel](main_classes/model.html#transformers.PreTrainedModel), and [PretrainedConfig](main_classes/configuration.html#transformers.PretrainedConfig). For exemplary purposes, we will
...
[PreTrainedModel](main_classes/model.html#transformers.PreTrainedModel). The important functionalities that are automatically provided to every new
model are [from_pretrained()](main_classes/model.html#transformers.PreTrainedModel.from_pretrained) and
[save_pretrained()](main_classes/model.html#transformers.PreTrainedModel.save_pretrained), which are used for serialization and deserialization. All of the
...
[PreTrainedModel](main_classes/model.html#transformers.PreTrainedModel), and thus can be accessed via the `config` attribute for all classes
...
[PretrainedConfig](main_classes/configuration.html#transformers.PretrainedConfig). Note that the configuration and the model are always serialized into two
different formats - the model to a _pytorch_model.bin_ file and the configuration to a _config.json_ file. Calling
[save_pretrained()](main_classes/model.html#transformers.PreTrainedModel.save_pretrained) will automatically call
[save_pretrained()](main_classes/configuration.html#transformers.PretrainedConfig.save_pretrained), so that both model and configuration are saved.
...
-  What type of model is *brand_new_bert*? BERT-like encoder-only model? GPT2-like decoder-only model? BART-like
  encoder-decoder model? Look at the [model_summary](model_summary.html) if you're not familiar with the differences between those.
...
), f"Pointer shape of random weight &lcubmodel_pointer.shape} and array shape of checkpoint weight &lcubpretrained_weight.shape} mismatched"
```

Besides, you should also print out the names of both weights to make sure they match, *e.g.*

```python
logger.info(f"Initialize PyTorch weight &lcublayer_name} from &lcubpretrained_weight.name}")
```
