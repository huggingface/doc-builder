...
a sequence length of \\(n_s = 2^&lcub19} \approx 0.5M\\) and a `config.hidden_size` of \\(d = 2^&lcub10} \approx 1000\\)
would result in a position encoding matrix:

$$X_&lcubi,j}, \text&lcub with } i \in \left[1,\ldots, d\right] \text&lcub and } j \in \left[1,\ldots, n_s\right]$$

which alone has over 500M parameters to store. Axial positional encodings factorize \\(X_&lcubi,j}\\) into two matrices:

$$X^&lcub1}_&lcubi,j}, \text&lcub with } i \in \left[1,\ldots, d^1\right] \text&lcub and } j \in \left[1,\ldots, n_s^1\right]$$

and

$$X^&lcub2}_&lcubi,j}, \text&lcub with } i \in \left[1,\ldots, d^2\right] \text&lcub and } j \in \left[1,\ldots, n_s^2\right]$$
...
$$X_&lcubi,j} = \begin&lcubcases}
X^&lcub1}_&lcubi, k}, & \text&lcubif }\ i &amp;lt; d^1 \text&lcub with } k = j \mod n_s^1 \\
X^&lcub2}_&lcubi - d^1, l}, & \text&lcubif } i \ge d^1 \text&lcub with } l = \lfloor\frac&lcubj}&lcubn_s^1}\rfloor
\end&lcubcases}$$

Intuitively, this means that a position embedding vector \\(x_j \in \mathbb&lcubR}^&lcubd}\\) is now the composition of two
factorized embedding vectors: \\(x^1_&lcubk, l} + x^2_&lcubl, k}\\), where as the `config.max_embedding_size` dimension
\\(j\\) is factorized into \\(k \text&lcub and } l\\). This design ensures that each position embedding vector
\\(x_j\\) is unique.

Using the above example again, axial position encoding with \\(d^1 = 2^5, d^2 = 2^5, n_s^1 = 2^9, n_s^2 = 2^&lcub10}\\)
can drastically reduced the number of parameters to \\(2^&lcub14} + 2^&lcub15} \approx 49000\\) parameters.
...
Note that `config.num_buckets` can also be factorized into a list \\((n_&lcub\text&lcubbuckets}}^1,
n_&lcub\text&lcubbuckets}}^2)\\). This way instead of assigning the query key embedding vectors to one of \\((1,\ldots,
n_&lcub\text&lcubbuckets}})\\) they are assigned to one of \\((1-1,\ldots, n_&lcub\text&lcubbuckets}}^1-1, \ldots,
1-n_&lcub\text&lcubbuckets}}^2, \ldots, n_&lcub\text&lcubbuckets}}^1-n_&lcub\text&lcubbuckets}}^2)\\). This is crucial for very long sequences to
save memory.
...
\\(\mathcal&lcubO}(n_s \times n_s)\\) to \\(\mathcal&lcubO}(n_s \times \log(n_s))\\), which usually represents the memory
...
For training, the [ReformerModelWithLMHead](model_doc/reformer.html#transformers.ReformerModelWithLMHead) should be used as follows:
...
This is the configuration class to store the configuration of a [ReformerModel](model_doc/reformer.html#transformers.ReformerModel). It is used to
...
- **is_decoder** (`bool`, _optional_, defaults to `False`) --
  ...
  [ReformerModel](model_doc/reformer.html#transformers.ReformerModel). When using the Reformer for causal language modeling, this argument
...
- **vocab_size** (`int`, _optional_, defaults to 320) --\
  ...
  the `inputs_ids` passed when calling [ReformerModel](model_doc/reformer.html#transformers.ReformerModel).
...
- **config** ([ReformerConfig](model_doc/reformer.html#transformers.ReformerConfig)) -- Model configuration class with all the parameters of the model.
...
The [ReformerModel](model_doc/reformer.html#transformers.ReformerModel) forward method, overrides the `__call__` special method.
...
- **input_ids** (`torch.LongTensor` of shape `(batch_size, sequence_length)`) --
  ...
  Indices can be obtained using [ReformerTokenizer](model_doc/reformer.html#transformers.ReformerTokenizer). See
...
- **num_hashes** (`int`, _optional_) --
  ...
  For more information, see `num_hashes` in [ReformerConfig](model_doc/reformer.html#transformers.ReformerConfig).
...
various elements depending on the configuration ([ReformerConfig](model_doc/reformer.html#transformers.ReformerConfig)) and inputs.
...
The [ReformerModelWithLMHead](model_doc/reformer.html#transformers.ReformerModelWithLMHead) forward method, overrides the `__call__` special method.
...
The [ReformerForMaskedLM](model_doc/reformer.html#transformers.ReformerForMaskedLM) forward method, overrides the `__call__` special method.
...
The [ReformerForSequenceClassification](model_doc/reformer.html#transformers.ReformerForSequenceClassification) forward method, overrides the `__call__` special method.
...
The [ReformerForQuestionAnswering](model_doc/reformer.html#transformers.ReformerForQuestionAnswering) forward method, overrides the `__call__` special method.
