...
- XLSR-Wav2Vec2 model was trained using connectionist temporal classification (CTC) so the model output has to be
  decoded using [Wav2Vec2CTCTokenizer](docs/transformers/:version/:language/model_doc/wav2vec2.html#transformers.Wav2Vec2CTCTokenizer).

XLSR-Wav2Vec2's architecture is based on the Wav2Vec2 model, so one can refer to [Wav2Vec2's documentation page](/docs/transformers/:version/:language/wav2vec2.html).
