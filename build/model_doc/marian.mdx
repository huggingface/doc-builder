...
- The modeling code is the same as [BartForConditionalGeneration](model_doc/bart.html#transformers.BartForConditionalGeneration) with a few minor modifications:
...
- All model names use the following format: `Helsinki-NLP/opus-mt-&lcubsrc}-&lcubtgt}`
- The language codes used to name models are inconsistent. Two digit codes can usually be found [here](https://developers.google.com/admin-sdk/directory/v1/languages), three digit codes require googling "language
  code &lcubcode}".
- Codes formatted like `es_AR` are usually `code_&lcubregion}`. That one is Spanish from Argentina.
...
old_style_multi_models = [f'&lcuborg}/&lcubs}' for s in suffix if s != s.lower()]
```
...
This is the configuration class to store the configuration of a [MarianModel](model_doc/marian.html#transformers.MarianModel). It is used to
...
- **vocab_size** (`int`, _optional_, defaults to 50265) --
  ...
  `inputs_ids` passed when calling [MarianModel](model_doc/marian.html#transformers.MarianModel) or
  [TFMarianModel](model_doc/marian.html#transformers.TFMarianModel).
...
- **config** ([MarianConfig](model_doc/marian.html#transformers.MarianConfig)) --
...
The [MarianModel](model_doc/marian.html#transformers.MarianModel) forward method, overrides the `__call__` special method.
...
- **input_ids** (`torch.LongTensor` of shape `(batch_size, sequence_length)`) --
  ...
  Indices can be obtained using [MarianTokenizer](model_doc/marian.html#transformers.MarianTokenizer). See
...
various elements depending on the configuration ([MarianConfig](model_doc/marian.html#transformers.MarianConfig)) and inputs.
...
The [MarianMTModel](model_doc/marian.html#transformers.MarianMTModel) forward method, overrides the `__call__` special method.
...
>>> model_name = f'Helsinki-NLP/opus-mt-&lcubsrc}-&lcubtrg}'
...
- **config** ([MarianConfig](model_doc/marian.html#transformers.MarianConfig)) -- Model configuration class with all the parameters of the model.
...
The [TFMarianModel](model_doc/marian.html#transformers.TFMarianModel) forward method, overrides the `__call__` special method.
...
elements depending on the configuration ([MarianConfig](model_doc/marian.html#transformers.MarianConfig)) and inputs.
...
The [TFMarianMTModel](model_doc/marian.html#transformers.TFMarianMTModel) forward method, overrides the `__call__` special method.
