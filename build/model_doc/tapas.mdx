...
- local: tapas-specific-outputs
  title: Tapas specific outputs
- local: tapasconfig
  title: TapasConfig
- local: tapastokenizer
  title: TapasTokenizer
- local: tapasmodel
  title: TapasModel
- local: tapasformaskedlm
  title: TapasForMaskedLM
- local: tapasforsequenceclassification
  title: TapasForSequenceClassification
- local: tapasforquestionanswering
...
- TAPAS is a model that uses relative position embeddings by default (restarting the position embeddings at every cell
  ...
  [TapasConfig](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasConfig), which is set to `True` by default. The default versions of the models available
...
## [Usage: fine-tuning](#usage-finetuning)

Here we explain how you can fine-tune [TapasForQuestionAnswering](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasForQuestionAnswering) on your own dataset.

**STEP 1: Choose one of the 3 ways in which you can use TAPAS - or experiment**

Basically, there are 3 different ways in which one can fine-tune [TapasForQuestionAnswering](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasForQuestionAnswering),
...
experiment by defining any hyperparameters you want when initializing [TapasConfig](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasConfig), and then
create a [TapasForQuestionAnswering](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasForQuestionAnswering) based on that configuration. For example, if you have a
...
data), you can then use [TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer) to convert table-question pairs into `input_ids`,
`attention_mask`, `token_type_ids` and so on. Again, based on which of the three cases you picked above,
[TapasForQuestionAnswering](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasForQuestionAnswering) requires different inputs to be fine-tuned:
...
[TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer) creates the `labels`, `numeric_values` and `numeric_values_scale` based on
...
Note that [TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer) expects the data of the table to be **text-only**. You can use
...
docs of [TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer)) are set correctly. See [this notebook](https://github.com/NielsRogge/Transformers-Tutorials/blob/master/TAPAS/Fine_tuning_TapasForQuestionAnswering_on_SQA.ipynb)
for more info.

**STEP 4: Train (fine-tune) TapasForQuestionAnswering**

You can then fine-tune [TapasForQuestionAnswering](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasForQuestionAnswering) using native PyTorch as follows (shown here for
...
Here we explain how you can use [TapasForQuestionAnswering](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasForQuestionAnswering) for inference (i.e. making predictions
on new data). For inference, only `input_ids`, `attention_mask` and `token_type_ids` (which you can obtain using
[TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer)) have to be provided to the model to obtain the logits. Next, you can use the
handy `convert_logits_to_predictions` method of [TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer) to convert these into predicted
...
Output type of [TapasForQuestionAnswering](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasForQuestionAnswering).
...
This is the configuration class to store the configuration of a [TapasModel](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasModel). It is used to
...
the model outputs. Read the documentation from [PretrainedConfig](/docs/transformers/master/en/main_classes/configuration.html#transformers.PretrainedConfig) for more information.
...
- **vocab_size** (`int`, _optional_, defaults to 30522) --
  ...
  `inputs_ids` passed when calling [TapasModel](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasModel).
...
- **type_vocab_sizes** (`List[int]`, _optional_, defaults to `[3, 256, 256, 2, 256, 256, 10]`) --
  The vocabulary sizes of the `token_type_ids` passed when calling [TapasModel](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasModel).
...
[TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer) creates several token type ids to encode tabular structure. To be more
...
[TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer) runs end-to-end tokenization on a table and associated sentences: punctuation
...
Converts logits of [TapasForQuestionAnswering](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasForQuestionAnswering) to actual predicted answer coordinates and
...
- **data** (`dict`) --
  ...
  [TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer).
...
- **config** ([TapasConfig](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasConfig)) -- Model configuration class with all the parameters of the model.
  ...
  This class is a small change compared to [BertModel](/docs/transformers/master/en/model_doc/bert.html#transformers.BertModel), taking into account the additional token
  type ids.
...
The [TapasModel](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasModel) forward method, overrides the `__call__` special method.
...
- **input_ids** (`torch.LongTensor` of shape `(batch_size, sequence_length)`) --
  ...
  [TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer). See [transformers.PreTrainedTokenizer.encode()](/docs/transformers/master/en/internal/tokenization_utils.html#transformers.PreTrainedTokenizerBase.encode) and
...
- **token_type_ids** (`torch.LongTensor` of shape `(batch_size, sequence_length, 7)`, _optional_) --
  ...
  [TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer). See this class for more info.
...
- **position_ids** (`torch.LongTensor` of shape `(batch_size, sequence_length)`, _optional_) --
  ...
  `reset_position_index_per_cell` of [TapasConfig](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasConfig) is set to `True`, relative
...
various elements depending on the configuration ([TapasConfig](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasConfig)) and inputs.
...
The [TapasForMaskedLM](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasForMaskedLM) forward method, overrides the `__call__` special method.
...
The [TapasForSequenceClassification](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasForSequenceClassification) forward method, overrides the `__call__` special method.
...
The [TapasForQuestionAnswering](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasForQuestionAnswering) forward method, overrides the `__call__` special method.
...
- **labels** (`torch.LongTensor` of shape `(batch_size, seq_length)`, _optional_) --
  ...
  answer appearing in the table. Can be obtained using [TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer).
...
- **numeric_values** (`torch.FloatTensor` of shape `(batch_size, seq_length)`, _optional_) --
  ...
  [TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer). Only required in case of weak supervision for aggregation (WTQ) to
  calculate the regression loss.
- **numeric_values_scale** (`torch.FloatTensor` of shape `(batch_size, seq_length)`, _optional_) --
  Scale of the numeric values of every token. Can be obtained using [TapasTokenizer](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasTokenizer).
  Only required in case of weak supervision for aggregation (WTQ) to calculate the regression loss.


> Returns

A [TableQuestionAnsweringOutput](/docs/transformers/master/en/model_doc/tapas.html#transformers.models.tapas.modeling_tapas.TableQuestionAnsweringOutput) or a tuple of
`torch.FloatTensor` (if `return_dict=False` is passed or when `config.return_dict=False`) comprising
various elements depending on the configuration ([TapasConfig](/docs/transformers/master/en/model_doc/tapas.html#transformers.TapasConfig)) and inputs.
...
[TableQuestionAnsweringOutput](/docs/transformers/master/en/model_doc/tapas.html#transformers.models.tapas.modeling_tapas.TableQuestionAnsweringOutput) or `tuple(torch.FloatTensor)`
