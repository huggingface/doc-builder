...
can be imported using `from_pretrained()` (see example in [quicktour](/docs/transformers/:version/:language/quicktour.html) , [run_glue.py](https://github.com/huggingface/transformers/tree/master/examples/pytorch/text-classification/run_glue.py) \ ).
