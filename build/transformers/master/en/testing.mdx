...
1. Once a PR is submitted it gets tested with 9 CircleCi jobs. Every new commit to that PR gets retested. These jobs
   are defined in this :prefix_link:_config file  , so that if needed you can reproduce the same
...
2. There are 3 jobs run by [github actions](https://github.com/huggingface/transformers/actions):
   ...
   - :prefix_link:_torch hub integration  : checks whether torch hub
     integration works.

   - :prefix_link:_self-hosted (push)  : runs fast tests on GPU only on commits on
   ...
   - :prefix_link:_self-hosted runner  : runs normal and slow tests on GPU in
     `tests` and `examples`:
...
- :prefix_link:_test_trainer_distributed.py  
- :prefix_link:_test_deepspeed.py  
...
Here is a an example of a :prefix_link:_script   that created the tiny model
