...
As we saw in [the preprocessing tutorial](/docs/transformers/master/en/preprocessing), tokenizing a text is splitting it into words or
...
(BPE)](/docs/transformers/master/en/tokenizer_summary#byte-pair-encoding), [WordPiece](/docs/transformers/master/en/tokenizer_summary#wordpiece), and [SentencePiece](/docs/transformers/master/en/tokenizer_summary#sentencepiece), and show examples
...
that the model uses [WordPiece](/docs/transformers/master/en/tokenizer_summary#wordpiece).
...
usually generates a very big vocabulary (the set of all unique words and tokens used). *E.g.*, [Transformer XL](/docs/transformers/master/en/model_doc/transformerxl) uses space and punctuation tokenization, resulting in a vocabulary size of 267,735!
...
We'll get back to the meaning of those `"▁"` when we look at [SentencePiece](/docs/transformers/master/en/tokenizer_summary#sentencepiece). As one can see,
...
words. Pretokenization can be as simple as space tokenization, e.g. [GPT-2](/docs/transformers/master/en/model_doc/gpt2), [Roberta](/docs/transformers/master/en/model_doc/roberta). More advanced pre-tokenization include rule-based tokenization, e.g. [XLM](/docs/transformers/master/en/model_doc/xlm),
[FlauBERT](/docs/transformers/master/en/model_doc/flaubert) which uses Moses for most languages, or [GPT](/docs/transformers/master/en/model_doc/gpt) which uses
...
to choose. For instance [GPT](/docs/transformers/master/en/model_doc/gpt) has a vocabulary size of 40,478 since they have 478 base characters
...
tokenizer can tokenize every text without the need for the &amp;lt;unk> symbol. [GPT-2](/docs/transformers/master/en/model_doc/gpt) has a vocabulary
...
WordPiece is the subword tokenization algorithm used for [BERT](/docs/transformers/master/en/model_doc/bert), [DistilBERT](/docs/transformers/master/en/model_doc/distilbert), and [Electra](/docs/transformers/master/en/model_doc/electra). The algorithm was outlined in [Japanese and Korean
...
conjunction with [SentencePiece](/docs/transformers/master/en/tokenizer_summary#sentencepiece).
...
specific pre-tokenizers, *e.g.* [XLM](/docs/transformers/master/en/model_doc/xlm) uses a specific Chinese, Japanese, and Thai pre-tokenizer).
...
using SentencePiece are [ALBERT](/docs/transformers/master/en/model_doc/albert), [XLNet](/docs/transformers/master/en/model_doc/xlnet), [Marian](/docs/transformers/master/en/model_doc/marian), and [T5](/docs/transformers/master/en/model_doc/t5).
