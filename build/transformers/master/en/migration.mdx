...
<h2 id="migrating-from-transformers-vx-to-vx">Migrating from transformers `v3.x` to `v4.x`</h2>

A couple of changes were introduced when the switch from version 3 to version 4 was done. Below is a summary of the
expected changes:

<h4 id="autotokenizers-and-pipelines-now-use-fast-rust-tokenizers-by-default">1. AutoTokenizers and pipelines now use fast (rust) tokenizers by default.</h4>
...
<h5 id="how-to-obtain-the-same-behavior-as-vx-in-vx">How to obtain the same behavior as v3.x in v4.x</h5>
...
<h4 id="sentencepiece-is-removed-from-the-required-dependencies">2. SentencePiece is removed from the required dependencies</h4>
...
pip install transformers sentencepiece
```
<h4 id="the-architecture-of-the-repo-has-been-updated-so-that-each-model-resides-in-its-folder">3. The architecture of the repo has been updated so that each model resides in its folder</h4>
...
<h4 id="switching-the-returndict-argument-to-true-by-default">4. Switching the `return_dict` argument to `True` by default</h4>
...
<h4 id="removed-some-deprecated-attributes">5. Removed some deprecated attributes</h4>
