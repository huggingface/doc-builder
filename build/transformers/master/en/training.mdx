...
negative. For examples of other tasks, refer to the [additional-resources](/docs/transformers/master/en/training#additional-resources) section!
...
to the 🤗 Datasets [documentation](https://huggingface.co/docs/datasets/) or the [preprocessing](/docs/transformers/master/en/preprocessing) tutorial for
more information.
...
As we saw in [preprocessing](/docs/transformers/master/en/preprocessing), we can prepare the text inputs for the model with the following command (this is an
...
Let's now see how to achieve the same results as in [trainer section](/docs/transformers/master/en/training#trainer) in PyTorch. First we need to
...
Now to check the results, we need to write the evaluation loop. Like in the [trainer section](/docs/transformers/master/en/training#trainer) we will
