...
page](pretrained_models.html) to see the checkpoints available for each type of model and all [the community models](https://huggingface.co/models).
...
<!--More information in this [model documentation](model_doc/mmbt.html.html). TODO: write this page
-->
...
dimensions \\(l_&lcub1} \times d_&lcub1}\\) and \\(l_&lcub2} \times d_&lcub2}\\), such that \\(l_&lcub1} \times l_&lcub2} = l\\) and
\\(d_&lcub1} + d_&lcub2} = d\\) (with the product for the lengths, this ends up being way smaller). The embedding for time
