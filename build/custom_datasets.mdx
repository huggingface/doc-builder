...
[Trainer](main_classes/trainer.html#transformers.Trainer)/[TFTrainer](main_classes/trainer.html#transformers.TFTrainer) or with native PyTorch/TensorFlow.
...
[forward()](model_doc/distilbert.html#transformers.DistilBertForSequenceClassification.forward) method of the model we will train.
...
item = &lcubkey: torch.tensor(val[idx]) for key, val in self.encodings.items()}
...
[Trainer](main_classes/trainer.html#transformers.Trainer)/[TFTrainer](main_classes/trainer.html#transformers.TFTrainer) or with native PyTorch/TensorFlow. See [training](training.html).
...
to fine-tune, define the [TrainingArguments](main_classes/trainer.html#transformers.TrainingArguments)/[TFTrainingArguments](main_classes/trainer.html#transformers.TFTrainingArguments) and
instantiate a [Trainer](main_classes/trainer.html#transformers.Trainer)/[TFTrainer](main_classes/trainer.html#transformers.TFTrainer).
...
tag2id = &lcubtag: id for id, tag in enumerate(unique_tags)}
id2tag = &lcubid: tag for tag, id in tag2id.items()}
```
...
[Trainer](main_classes/trainer.html#transformers.Trainer)/[TFTrainer](main_classes/trainer.html#transformers.TFTrainer) or with native PyTorch/TensorFlow, exactly as in the
...
we can use the built in [char_to_token()](main_classes/tokenizer.html#transformers.BatchEncoding.char_to_token) method.
...
def __getitem__(self, idx):
return &lcubkey: torch.tensor(val[idx]) for key, val in self.encodings.items()}
...
train_dataset = tf.data.Dataset.from_tensor_slices((
&lcubkey: train_encodings[key] for key in ['input_ids', 'attention_mask']},
&lcubkey: train_encodings[key] for key in ['start_positions', 'end_positions']}
))
val_dataset = tf.data.Dataset.from_tensor_slices((
&lcubkey: val_encodings[key] for key in ['input_ids', 'attention_mask']},
&lcubkey: val_encodings[key] for key in ['start_positions', 'end_positions']}
))
```
{/if}
...
[Trainer](main_classes/trainer.html#transformers.Trainer)/[TFTrainer](main_classes/trainer.html#transformers.TFTrainer) exactly as in the sequence classification example
...
- [Preprocessing](preprocessing.html). Docs page on data preprocessing.
- [Training](training.html). Docs page on training and fine-tuning.
...
>>> &lcubkey: val.shape for key, val in train[0].items()})
