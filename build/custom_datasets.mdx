---
local: finetuning-with-custom-datasets
sections:
- local: sequence-classification-with-imdb-reviews
  sections:
  - local: finetuning-with-trainer
    title: Fine-tuning with Trainer
  - local: finetuning-with-native-pytorchtensorflow
    title: Fine-tuning with native PyTorch/TensorFlow
  title: Sequence Classification with IMDb Reviews
- local: token-classification-with-wnut-emerging-entities
  title: Token Classification with W-NUT Emerging Entities
- local: question-answering-with-squad
  title: Question Answering with SQuAD 2.0
- local: additional-resources
  sections:
  - local: using-the-datasets-metrics-library
    title: Using the 🤗 Datasets & Metrics library
...
# [Fine-tuning with custom datasets](#finetuning-with-custom-datasets)

<Tip>

The datasets used in this tutorial are available and can be more easily accessed using the [🤗 Datasets library](https://github.com/huggingface/datasets). We do not use this library to access the datasets here since this
tutorial meant to illustrate how to work with your own data. A brief of introduction can be found at the end of the
tutorial in the section "[datasetslib](/docs/transformers/master/en/custom_datasets.html#datasetslib)".
...
[Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer)/[TFTrainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.TFTrainer) or with native PyTorch/TensorFlow.

We include several examples, each of which demonstrates a different type of common downstream task:

- [seq_imdb](/docs/transformers/master/en/custom_datasets.html#seq_imdb)
- [tok_ner](/docs/transformers/master/en/custom_datasets.html#tok_ner)
- [qa_squad](/docs/transformers/master/en/custom_datasets.html#qa_squad)
- [resources](/docs/transformers/master/en/custom_datasets.html#resources)
...
This dataset can be explored in the Hugging Face model hub ([IMDb](https://huggingface.co/datasets/imdb)), and
...
[forward()](/docs/transformers/master/en/model_doc/distilbert.html#transformers.DistilBertForSequenceClassification.forward) method of the model we will train.
...
item = &amp;lcub;key: torch.tensor(val[idx]) for key, val in self.encodings.items()}
...
[Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer)/[TFTrainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.TFTrainer) or with native PyTorch/TensorFlow. See [training](/docs/transformers/master/en/training.html).

<a id='ft_trainer'></a>

### [Fine-tuning with Trainer](#finetuning-with-trainer)

The steps above prepared the datasets in the way that the trainer is expected. Now all we need to do is create a model
to fine-tune, define the [TrainingArguments](/docs/transformers/master/en/main_classes/trainer.html#transformers.TrainingArguments)/[TFTrainingArguments](/docs/transformers/master/en/main_classes/trainer.html#transformers.TFTrainingArguments) and
instantiate a [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer)/[TFTrainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.TFTrainer).
...
### [Fine-tuning with native PyTorch/TensorFlow](#finetuning-with-native-pytorchtensorflow)
...
## [Token Classification with W-NUT Emerging Entities](#token-classification-with-wnut-emerging-entities)

<Tip>

This dataset can be explored in the Hugging Face model hub ([WNUT-17](https://huggingface.co/datasets/wnut_17)),
...
tag2id = &amp;lcub;tag: id for id, tag in enumerate(unique_tags)}
id2tag = &amp;lcub;id: tag for tag, id in tag2id.items()}
```
...
vocabulary. For example, DistilBert's tokenizer would split the Twitter handle `@huggingface` into the tokens `['@', 'hugging', '##face']`. This is a problem for us because we have exactly one tag per token. If the tokenizer splits a
...
[Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer)/[TFTrainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.TFTrainer) or with native PyTorch/TensorFlow, exactly as in the
sequence classification example above.

- [ft_trainer](/docs/transformers/master/en/custom_datasets.html#ft_trainer)
- [ft_native](/docs/transformers/master/en/custom_datasets.html#ft_native)

<a id='qa_squad'></a>

## [Question Answering with SQuAD 2.0](#question-answering-with-squad)

<Tip>

This dataset can be explored in the Hugging Face model hub ([SQuAD V2](https://huggingface.co/datasets/squad_v2)), and can be alternatively downloaded with the 🤗 Datasets library with
...
we can use the built in [char_to_token()](/docs/transformers/master/en/main_classes/tokenizer.html#transformers.BatchEncoding.char_to_token) method.
...
encodings.update(&amp;lcub;'start_positions': start_positions, 'end_positions': end_positions})
...
def __getitem__(self, idx):
return &amp;lcub;key: torch.tensor(val[idx]) for key, val in self.encodings.items()}
...
train_dataset = tf.data.Dataset.from_tensor_slices((
&amp;lcub;key: train_encodings[key] for key in ['input_ids', 'attention_mask']},
&amp;lcub;key: train_encodings[key] for key in ['start_positions', 'end_positions']}
))
val_dataset = tf.data.Dataset.from_tensor_slices((
&amp;lcub;key: val_encodings[key] for key in ['input_ids', 'attention_mask']},
&amp;lcub;key: val_encodings[key] for key in ['start_positions', 'end_positions']}
))
```
{/if}
...
[Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer)/[TFTrainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.TFTrainer) exactly as in the sequence classification example
...
- [Preprocessing](/docs/transformers/master/en/preprocessing.html). Docs page on data preprocessing.
- [Training](/docs/transformers/master/en/training.html). Docs page on training and fine-tuning.

<a id='datasetslib'></a>

### [Using the 🤗 Datasets & Metrics library](#using-the-datasets-metrics-library)
...
>>> &amp;lcub;key: val.shape for key, val in train[0].items()})
&amp;lcub;'labels': torch.Size([]), 'input_ids': torch.Size([512]), 'attention_mask': torch.Size([512])}
```
{:else}
```python
>>> train.set_format("tensorflow", columns=["input_ids", "attention_mask", "labels"])
>>> &amp;lcub;key: val.shape for key, val in train[0].items()})
&amp;lcub;'labels': TensorShape([]), 'input_ids': TensorShape([512]), 'attention_mask': TensorShape([512])}
```
{/if}
