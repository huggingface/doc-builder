...
As we saw in [the preprocessing tutorial](/docs/transformers/:version/:language/preprocessing.html), tokenizing a text is splitting it into words or
...
type was used by the pretrained model. For instance, if we look at [BertTokenizer](docs/transformers/:version/:language/model_doc/bert.html#transformers.BertTokenizer), we can see
...
usually generates a very big vocabulary (the set of all unique words and tokens used). *E.g.*, [Transformer XL](/docs/transformers/:version/:language/model_doc/transformerxl.html) uses space and punctuation tokenization, resulting in a vocabulary size of 267,735!
...
seen before, by decomposing them into known subwords. For instance, the [BertTokenizer](docs/transformers/:version/:language/model_doc/bert.html#transformers.BertTokenizer) tokenizes
...
As another example, [XLNetTokenizer](docs/transformers/:version/:language/model_doc/xlnet.html#transformers.XLNetTokenizer) tokenizes our previously exemplary text as follows:
...
words. Pretokenization can be as simple as space tokenization, e.g. [GPT-2](/docs/transformers/:version/:language/model_doc/gpt2.html), [Roberta](/docs/transformers/:version/:language/model_doc/roberta.html). More advanced pre-tokenization include rule-based tokenization, e.g. [XLM](/docs/transformers/:version/:language/model_doc/xlm.html),
[FlauBERT](/docs/transformers/:version/:language/model_doc/flaubert.html) which uses Moses for most languages, or [GPT](/docs/transformers/:version/:language/model_doc/gpt.html) which uses
...
to choose. For instance [GPT](/docs/transformers/:version/:language/model_doc/gpt.html) has a vocabulary size of 40,478 since they have 478 base characters
...
tokenizer can tokenize every text without the need for the &amp;lt;unk> symbol. [GPT-2](/docs/transformers/:version/:language/model_doc/gpt.html) has a vocabulary
...
WordPiece is the subword tokenization algorithm used for [BERT](/docs/transformers/:version/:language/model_doc/bert.html), [DistilBERT](/docs/transformers/:version/:language/model_doc/distilbert.html), and [Electra](/docs/transformers/:version/:language/model_doc/electra.html). The algorithm was outlined in [Japanese and Korean
...
specific pre-tokenizers, *e.g.* [XLM](/docs/transformers/:version/:language/model_doc/xlm.html) uses a specific Chinese, Japanese, and Thai pre-tokenizer).
...
The [XLNetTokenizer](docs/transformers/:version/:language/model_doc/xlnet.html#transformers.XLNetTokenizer) uses SentencePiece for example, which is also why in the example earlier the
...
using SentencePiece are [ALBERT](/docs/transformers/:version/:language/model_doc/albert.html), [XLNet](/docs/transformers/:version/:language/model_doc/xlnet.html), [Marian](/docs/transformers/:version/:language/model_doc/marian.html), and [T5](/docs/transformers/:version/:language/model_doc/t5.html).
