...
As we saw in [the preprocessing tutorial](preprocessing.html), tokenizing a text is splitting it into words or
...
type was used by the pretrained model. For instance, if we look at [BertTokenizer](model_doc/bert.html#transformers.BertTokenizer), we can see
...
usually generates a very big vocabulary (the set of all unique words and tokens used). *E.g.*, [Transformer XL](model_doc/transformerxl.html) uses space and punctuation tokenization, resulting in a vocabulary size of 267,735!
...
seen before, by decomposing them into known subwords. For instance, the [BertTokenizer](model_doc/bert.html#transformers.BertTokenizer) tokenizes
...
As another example, [XLNetTokenizer](model_doc/xlnet.html#transformers.XLNetTokenizer) tokenizes our previously exemplary text as follows:
...
words. Pretokenization can be as simple as space tokenization, e.g. [GPT-2](model_doc/gpt2.html), [Roberta](model_doc/roberta.html). More advanced pre-tokenization include rule-based tokenization, e.g. [XLM](model_doc/xlm.html),
[FlauBERT](model_doc/flaubert.html) which uses Moses for most languages, or [GPT](model_doc/gpt.html) which uses
...
to choose. For instance [GPT](model_doc/gpt.html) has a vocabulary size of 40,478 since they have 478 base characters
...
tokenizer can tokenize every text without the need for the &amp;lt;unk> symbol. [GPT-2](model_doc/gpt.html) has a vocabulary
...
WordPiece is the subword tokenization algorithm used for [BERT](model_doc/bert.html), [DistilBERT](model_doc/distilbert.html), and [Electra](model_doc/electra.html). The algorithm was outlined in [Japanese and Korean
...
the words \\(x_&lcub1}, \dots, x_&lcubN}\\) and that the set of all possible tokenizations for a word \\(x_&lcubi}\\) is
defined as \\(S(x_&lcubi})\\), then the overall loss is defined as

$$\mathcal&lcubL} = -\sum_&lcubi=1}^&lcubN} \log \left ( \sum_&lcubx \in S(x_&lcubi})} p(x) \right )$$
...
specific pre-tokenizers, *e.g.* [XLM](model_doc/xlm.html) uses a specific Chinese, Japanese, and Thai pre-tokenizer).
...
The [XLNetTokenizer](model_doc/xlnet.html#transformers.XLNetTokenizer) uses SentencePiece for example, which is also why in the example earlier the
...
using SentencePiece are [ALBERT](model_doc/albert.html), [XLNet](model_doc/xlnet.html), [Marian](model_doc/marian.html), and [T5](model_doc/t5.html).
