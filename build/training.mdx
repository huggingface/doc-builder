...
the 🤗 Transformers library provides an API with the class [Trainer](docs/transformers/:version/:language/main_classes/trainer.html#transformers.Trainer) to let you fine-tune or train
...
to the 🤗 Datasets [documentation](https://huggingface.co/docs/datasets/) or the [preprocessing](/docs/transformers/:version/:language/preprocessing.html) tutorial for
more information.
...
As we saw in [preprocessing](/docs/transformers/:version/:language/preprocessing.html), we can prepare the text inputs for the model with the following command (this is an
...
Since PyTorch does not provide a training loop, the 🤗 Transformers library provides a [Trainer](docs/transformers/:version/:language/main_classes/trainer.html#transformers.Trainer)
...
Then, to define our [Trainer](docs/transformers/:version/:language/main_classes/trainer.html#transformers.Trainer), we will need to instantiate a
[TrainingArguments](docs/transformers/:version/:language/main_classes/trainer.html#transformers.TrainingArguments). This class contains all the hyperparameters we can tune for the
[Trainer](docs/transformers/:version/:language/main_classes/trainer.html#transformers.Trainer) or the flags to activate the different training options it supports. Let's begin by
...
Then we can instantiate a [Trainer](docs/transformers/:version/:language/main_classes/trainer.html#transformers.Trainer) like this:
...
[Trainer](docs/transformers/:version/:language/main_classes/trainer.html#transformers.Trainer) to compute any metrics. Let's have a look on how to do that now!

To have the [Trainer](docs/transformers/:version/:language/main_classes/trainer.html#transformers.Trainer) compute and report metrics, we need to give it a `compute_metrics`
function that takes predictions and labels (grouped in a namedtuple called [EvalPrediction](docs/transformers/:version/:language/internal/trainer_utils.html#transformers.EvalPrediction)) and
...
To check if this works on practice, let's create a new [Trainer](docs/transformers/:version/:language/main_classes/trainer.html#transformers.Trainer) with our fine-tuned model:
...
See the documentation of [TrainingArguments](docs/transformers/:version/:language/main_classes/trainer.html#transformers.TrainingArguments) for more options.
...
scheduler. The default optimizer used by the [Trainer](docs/transformers/:version/:language/main_classes/trainer.html#transformers.Trainer) is [AdamW](docs/transformers/:version/:language/main_classes/optimizer_schedules.html#transformers.AdamW):
