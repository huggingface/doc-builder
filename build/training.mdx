...
the 🤗 Transformers library provides an API with the class [Trainer](main_classes/trainer.html#transformers.Trainer) to let you fine-tune or train
...
to the 🤗 Datasets [documentation](https://huggingface.co/docs/datasets/) or the [preprocessing](preprocessing.html) tutorial for
more information.
...
As we saw in [preprocessing](preprocessing.html), we can prepare the text inputs for the model with the following command (this is an
...
Since PyTorch does not provide a training loop, the 🤗 Transformers library provides a [Trainer](main_classes/trainer.html#transformers.Trainer)
...
Then, to define our [Trainer](main_classes/trainer.html#transformers.Trainer), we will need to instantiate a
[TrainingArguments](main_classes/trainer.html#transformers.TrainingArguments). This class contains all the hyperparameters we can tune for the
[Trainer](main_classes/trainer.html#transformers.Trainer) or the flags to activate the different training options it supports. Let's begin by
...
Then we can instantiate a [Trainer](main_classes/trainer.html#transformers.Trainer) like this:
...
[Trainer](main_classes/trainer.html#transformers.Trainer) to compute any metrics. Let's have a look on how to do that now!

To have the [Trainer](main_classes/trainer.html#transformers.Trainer) compute and report metrics, we need to give it a `compute_metrics`
function that takes predictions and labels (grouped in a namedtuple called [EvalPrediction](internal/trainer_utils.html#transformers.EvalPrediction)) and
...
To check if this works on practice, let's create a new [Trainer](main_classes/trainer.html#transformers.Trainer) with our fine-tuned model:
...
See the documentation of [TrainingArguments](main_classes/trainer.html#transformers.TrainingArguments) for more options.
...
```python
train_features = &lcubx: tf_train_dataset[x].to_tensor() for x in tokenizer.model_input_names}
train_tf_dataset = tf.data.Dataset.from_tensor_slices((train_features, tf_train_dataset["label"]))
train_tf_dataset = train_tf_dataset.shuffle(len(tf_train_dataset)).batch(8)

eval_features = &lcubx: tf_eval_dataset[x].to_tensor() for x in tokenizer.model_input_names}
...
scheduler. The default optimizer used by the [Trainer](main_classes/trainer.html#transformers.Trainer) is [AdamW](main_classes/optimizer_schedules.html#transformers.AdamW):
...
batch = &lcubk: v.to(device) for k, v in batch.items()}
outputs = model(**batch)
loss = outputs.loss
loss.backward()
