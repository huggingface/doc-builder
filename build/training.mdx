...
# [Fine-tuning a pretrained model](#finetuning-a-pretrained-model)
...
the 🤗 Transformers library provides an API with the class [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) to let you fine-tune or train
...
negative. For examples of other tasks, refer to the [additional-resources](/docs/transformers/master/en/training.html#additional-resources) section!
...
to the 🤗 Datasets [documentation](https://huggingface.co/docs/datasets/) or the [preprocessing](/docs/transformers/master/en/preprocessing.html) tutorial for
more information.
...
As we saw in [preprocessing](/docs/transformers/master/en/preprocessing.html), we can prepare the text inputs for the model with the following command (this is an
...
## [Fine-tuning in PyTorch with the Trainer API](#finetuning-in-pytorch-with-the-trainer-api)

<iframe width="560" height="315" src="https://www.youtube.com/embed/nvBXf7s7vTI" title="YouTube video player"
frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope;
picture-in-picture" allowfullscreen></iframe>

Since PyTorch does not provide a training loop, the 🤗 Transformers library provides a [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer)
...
Then, to define our [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer), we will need to instantiate a
[TrainingArguments](/docs/transformers/master/en/main_classes/trainer.html#transformers.TrainingArguments). This class contains all the hyperparameters we can tune for the
[Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) or the flags to activate the different training options it supports. Let's begin by
...
Then we can instantiate a [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) like this:
...
[Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) to compute any metrics. Let's have a look on how to do that now!

To have the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) compute and report metrics, we need to give it a `compute_metrics`
function that takes predictions and labels (grouped in a namedtuple called [EvalPrediction](/docs/transformers/master/en/internal/trainer_utils.html#transformers.EvalPrediction)) and
...
To check if this works on practice, let's create a new [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) with our fine-tuned model:
...
See the documentation of [TrainingArguments](/docs/transformers/master/en/main_classes/trainer.html#transformers.TrainingArguments) for more options.


<a id='keras'></a>

## [Fine-tuning with Keras](#finetuning-with-keras)
...
```python
train_features = &amp;lcub;x: tf_train_dataset[x].to_tensor() for x in tokenizer.model_input_names}
train_tf_dataset = tf.data.Dataset.from_tensor_slices((train_features, tf_train_dataset["label"]))
train_tf_dataset = train_tf_dataset.shuffle(len(tf_train_dataset)).batch(8)

eval_features = &amp;lcub;x: tf_eval_dataset[x].to_tensor() for x in tokenizer.model_input_names}
...
## [Fine-tuning in native PyTorch](#finetuning-in-native-pytorch)
...
Let's now see how to achieve the same results as in [trainer section](/docs/transformers/master/en/training.html#trainer) in PyTorch. First we need to
...
scheduler. The default optimizer used by the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) is [AdamW](/docs/transformers/master/en/main_classes/optimizer_schedules.html#transformers.AdamW):
...
Now to check the results, we need to write the evaluation loop. Like in the [trainer section](/docs/transformers/master/en/training.html#trainer) we will
...
- [🤗 Transformers Notebooks](notebooks.html) which contains various notebooks and in particular one per task (look
  for the _how to finetune a model on xxx_).
