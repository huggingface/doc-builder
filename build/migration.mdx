...
sections:
- local: migrating-from-transformers-vx-to-vx
  sections:
  - local: autotokenizers-and-pipelines-now-use-fast-rust-tokenizers-by-default
    sections:
    - local: how-to-obtain-the-same-behavior-as-vx-in-vx
  ...
  - local: sentencepiece-is-removed-from-the-required-dependencies
    sections:
    - local: how-to-obtain-the-same-behavior-as-vx-in-vx
      title: How to obtain the same behavior as v3.x in v4.x
    title: 2. SentencePiece is removed from the required dependencies
  - local: the-architecture-of-the-repo-has-been-updated-so-that-each-model-resides-in-its-folder
    sections:
    - local: how-to-obtain-the-same-behavior-as-vx-in-vx
      title: How to obtain the same behavior as v3.x in v4.x
    title: 3. The architecture of the repo has been updated so that each model resides
      in its folder
  - local: switching-the-returndict-argument-to-true-by-default
    sections:
    - local: how-to-obtain-the-same-behavior-as-vx-in-vx
      title: How to obtain the same behavior as v3.x in v4.x
    title: 4. Switching the `return_dict` argument to `True` by default
  - local: removed-some-deprecated-attributes
...
# [Migrating from previous packages](#migrating-from-previous-packages)

## [Migrating from transformers `v3.x` to `v4.x`](#migrating-from-transformers-vx-to-vx)

A couple of changes were introduced when the switch from version 3 to version 4 was done. Below is a summary of the
expected changes:

#### [1. AutoTokenizers and pipelines now use fast (rust) tokenizers by default.](#autotokenizers-and-pipelines-now-use-fast-rust-tokenizers-by-default)
...
##### [How to obtain the same behavior as v3.x in v4.x](#how-to-obtain-the-same-behavior-as-vx-in-vx)
...
#### [2. SentencePiece is removed from the required dependencies](#sentencepiece-is-removed-from-the-required-dependencies)
...
pip install transformers sentencepiece
```
#### [3. The architecture of the repo has been updated so that each model resides in its folder](#the-architecture-of-the-repo-has-been-updated-so-that-each-model-resides-in-its-folder)
...
#### [4. Switching the `return_dict` argument to `True` by default](#switching-the-returndict-argument-to-true-by-default)
...
#### [5. Removed some deprecated attributes](#removed-some-deprecated-attributes)
...
## [Migrating from pytorch-transformers to 🤗 Transformers](#migrating-from-pytorchtransformers-to-transformers)

Here is a quick summary of what you should take care of when migrating from `pytorch-transformers` to 🤗 Transformers.

### [Positional order of some models' keywords inputs (`attention_mask`, `token_type_ids`...) changed](#positional-order-of-some-models-keywords-inputs-attentionmask-tokentypeids-changed)
...
## [Migrating from pytorch-pretrained-bert](#migrating-from-pytorchpretrainedbert)
