...
- local: trainer-deepspeed-integration
  ...
  - local: zero-and-infinity-nuances
    sections:
...
1. Integration of the core DeepSpeed features via [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer). This is everything done for you type
...
2. If you don't use [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) and want to use your own Trainer where you integrated DeepSpeed
   ...
   [deepspeed-non-trainer-integration](/docs/transformers/master/en/main_classes/deepspeed.html#deepspeed-non-trainer-integration).
...
If you're still struggling with the build, first make sure to read [zero-install-notes](/docs/transformers/master/en/main_classes/deepspeed.html#zero-install-notes).
...
To deploy this feature with multiple GPUs adjust the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) command line arguments as
following:
...
To deploy DeepSpeed with one GPU adjust the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) command line arguments as following:
...
When you execute the program, DeepSpeed will log the configuration it received from the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer)
...
[Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) via [TrainingArguments](/docs/transformers/master/en/main_classes/trainer.html#transformers.TrainingArguments) then for the `deepspeed` argument you can
pass a nested `dict`. This allows you to create the configuration on the fly and doesn't require you to write it to
the file system before passing it to [TrainingArguments](/docs/transformers/master/en/main_classes/trainer.html#transformers.TrainingArguments).
...
Some configuration values are required by both the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) and DeepSpeed to function correctly,
therefore, to prevent conflicting definitions, which could lead to hard to detect errors, we chose to configure those
via the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) command line arguments.

Additionally, some configuration values are derived automatically based on the model's configuration, so instead of
remembering to manually adjust multiple values, it's the best to let the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) do the majority
...
[Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) arguments and DeepSpeed configurations agree. For example, are you using the same
...
and configure [TrainingArguments](/docs/transformers/master/en/main_classes/trainer.html#transformers.TrainingArguments) based on that. The steps are:

1. Create or load the DeepSpeed configuration to be used as a master configuration
2. Create the [TrainingArguments](/docs/transformers/master/en/main_classes/trainer.html#transformers.TrainingArguments) object based on these values

Do note that some values, such as `scheduler.params.total_num_steps` are calculated by
[Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) during `train`, but you can of course do the math yourself.
...
This section has to be configured exclusively via DeepSpeed configuration - the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) provides
...
therefore set these values to `auto` and the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) will automatically assign the recommended
...
If you don't configure the `optimizer` entry in the configuration file, the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) will
...
But then you're on your own synchronizing the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) command line arguments and the DeepSpeed
configuration.
...
If you don't configure the `scheduler` entry in the configuration file, the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) will use
...
Since _"auto"_ is used the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) arguments will set the correct values in the configuration
...
and the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) will automatically enable or disable it based on the value of
...
and the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) will automatically configure it based on the values of `args.fp16_backend` and
`args.fp16_opt_level`.
...
and the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) will automatically set `train_micro_batch_size_per_gpu` to the value of
...
and the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) will automatically set it to the value of `args.gradient_accumulation_steps`.
...
and the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) will automatically set it to the value of `args.max_grad_norm`.
...
### [ZeRO-3 and Infinity Nuances](#zero-and-infinity-nuances)
...
section. Thus you must create the [TrainingArguments](/docs/transformers/master/en/main_classes/trainer.html#transformers.TrainingArguments) object **before** calling
...
`torch_dtype=torch.float16`. For details, please, see [from_pretrained-torch-dtype](/docs/transformers/master/en/main_classes/deepspeed.html#from_pretrained-torch-dtype).
...
2. either the command line arguments if you were using the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) or
   [TrainingArguments](/docs/transformers/master/en/main_classes/trainer.html#transformers.TrainingArguments) arguments if you were scripting the Trainer setup yourself. Please do not
   dump the [TrainingArguments](/docs/transformers/master/en/main_classes/trainer.html#transformers.TrainingArguments) as it has dozens of entries that are irrelevant.
...
- DeepSpeed works with the PyTorch [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) but not TF [TFTrainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.TFTrainer).
...
- You don't have to use the [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) to use DeepSpeed with 🤗 Transformers - you can use any model
...
functionality, when [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) is not used.

When using [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) everything is automatically taken care of.

When not using [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer), to efficiently deploy DeepSpeed stage 3, you must instantiate the
...
<a id='transformers.deepspeed.HfDeepSpeedConfig'></a>
> **class transformers.deepspeed.HfDeepSpeedConfig**(config_file_or_dict)
...
[Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) uses the `HfTrainerDeepSpeedConfig` subclass instead. That subclass has logic to
sync the configuration with values of [TrainingArguments](/docs/transformers/master/en/main_classes/trainer.html#transformers.TrainingArguments) by replacing special placeholder
...
<a id='transformers.deepspeed.HfDeepSpeedConfig.get_value'></a>
> **get\_value**(self, ds_key_long, default = None)


Returns the set value or `default` if no value is set


<a id='transformers.deepspeed.HfDeepSpeedConfig.is_false'></a>
> **is\_false**(self, ds_key_long)
...
<a id='transformers.deepspeed.HfDeepSpeedConfig.is_true'></a>
> **is\_true**(self, ds_key_long)
...
Finally, please, remember that, HuggingFace [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer) only integrates DeepSpeed, therefore if you
