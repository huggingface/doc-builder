...
tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', unk_token='&lcubunk>')
# You should be sure '&lcubunk>' is in the vocabulary when doing that.
# Otherwise use tokenizer.add_special_tokens({'unk_token': '&lcubunk>'}) instead)
assert tokenizer.unk_token == '&lcubunk>'
```
...
is also registered to be `'&lcub/s>'`).
