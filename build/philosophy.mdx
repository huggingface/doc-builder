...
- Be as easy and fast to use as possible:
  ...
  - We strongly limited the number of user-facing abstractions to learn, in fact, there are almost no abstractions,
    just three standard classes required to use each model: [configuration](/docs/transformers/master/en/main_classes/configuration.html),
    [models](/docs/transformers/master/en/main_classes/model.html) and [tokenizer](/docs/transformers/master/en/main_classes/tokenizer.html).
  ...
  - On top of those three base classes, the library provides two APIs: [pipeline()](/docs/transformers/master/en/main_classes/pipelines.html#transformers.pipeline) for quickly
    using a model (plus its associated tokenizer and configuration) on a given task and
    [Trainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.Trainer)/[TFTrainer](/docs/transformers/master/en/main_classes/trainer.html#transformers.TFTrainer) to quickly train or fine-tune a given model.
...
- **Model classes** such as [BertModel](/docs/transformers/master/en/model_doc/bert.html#transformers.BertModel), which are 30+ PyTorch models ([torch.nn.Module](https://pytorch.org/docs/stable/nn.html#torch.nn.Module)) or Keras models ([tf.keras.Model](https://www.tensorflow.org/api_docs/python/tf/keras/Model)) that work with the pretrained weights provided in the
  library.
- **Configuration classes** such as [BertConfig](/docs/transformers/master/en/model_doc/bert.html#transformers.BertConfig), which store all the parameters required to build
...
- **Tokenizer classes** such as [BertTokenizer](/docs/transformers/master/en/model_doc/bert.html#transformers.BertTokenizer), which store the vocabulary for each model and
...
- `from_pretrained()` lets you instantiate a model/configuration/tokenizer from a pretrained version either
  provided by the library itself (the supported models are provided in the list [here](/docs/transformers/master/en/pretrained_models.html)) or
