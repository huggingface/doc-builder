...
...     print(f"&lcubclasses[i]}: &lcubint(round(paraphrase_results[i] * 100))}%")
not paraphrase: 10%
is paraphrase: 90%

>>> # Should not be paraphrase
>>> for i in range(len(classes)):
...     print(f"&lcubclasses[i]}: &lcubint(round(not_paraphrase_results[i] * 100))}%")
not paraphrase: 94%
is paraphrase: 6%
```
{:else}
...
>>> print(f"Answer: '&lcubresult['answer']}', score: &lcubround(result['score'], 4)}, start: &lcubresult['start']}, end: &lcubresult['end']}")
...
...
...     print(f"Question: &lcubquestion}")
...     print(f"Answer: &lcubanswer}")
...
>>> pprint(unmasker(f"HuggingFace is creating a &lcubunmasker.tokenizer.mask_token} that the community uses to solve NLP tasks."))
[&lcub'score': 0.1793,
...
...     f"versions would help &lcubtokenizer.mask_token} our carbon footprint."
