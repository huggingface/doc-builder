			...
			if (docstringBody.match(REGEX_PARAMSGROUPS)) {
				...
				if (nParamGroups > 0) {
					...
					for (let groupId = 1; groupId <= nParamGroups; groupId++) {
						const REGEX_GROUP_TITLE =  new RegExp(`<paramsdesc${groupId}title>(((?!<paramsdesc${groupId}title>).)*)</paramsdesc${groupId}title>`, "ms");
						const REGEX_GROUP_CONTENT =  new RegExp(`<paramsdesc${groupId}>(((?!<paramsdesc${groupId}>).)*)</paramsdesc${groupId}>`, "ms");
 ...
 */
 function renderCode(code) {
	...
	return code.replace(REGEX_CODE_TAG, (_, group1) => `<code>${group1.replace(/&amp;/gm, "&")}</code>`);
}
		...
		highlighter: function(code, lang) {
			...
			if (code.match(REGEX_FRAMEWORKS_SPLIT)) {
				...
				if (codePt.match(REGEX_CODE_INPUT)) {
					codePt = codePt.split("\n").filter(line => line.match(REGEX_CODE_INPUT)).map(line => line.slice(4)).join("\n");
				}
 ...
 */
 function headingWithAnchorLink(code) {
	...
	return code.replace(REGEX_HEADER, (_, level, __, id, text) => {
		if (!id) {
			id = text.replace(/(.*)<a href.+>(.+)<\/a>(.*)/gm, (_, g1, g2, g3) => `${g1}${g2}${g3}`).toLowerCase().split(" ").join("-");
		}
